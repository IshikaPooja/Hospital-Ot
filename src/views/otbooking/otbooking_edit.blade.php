                        ...
                        <div class="card-body">
                            ...
                            <form   action="{{route('OT.otupdate')}}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="mb-3 row">                                  
                                    ...
                                    <input class="form-control" type="hidden" name="id"  id="otbook_id" value="{{$data->id}}"  >                        
                                        <input class="form-control" type="text" name="patient"  value="{{$data->patient}}" placeholder="Enter Patient name" required>                        
                                    ...
                                    <div class="col-sm-6  mt-3">
                                      <input class="form-control" type="text" name="ot_price" id="OTPrice" value="{{$data->ot_price}}" required>
                                      <span class="text-danger"> @error('ot_price') {{$message}}  @enderror  </span>
                                    </div>
                                    <div class="col-sm-6  mt-3">
                                      <input class="form-control" type="text" name="requested_by"  value="{{$data->requested_by}}" placeholder="Referd by" required>
                                      <span class="text-danger"> @error('requested_by') {{$message}}  @enderror  </span>
                                    </div>
                                    <div class="col-sm-6 mt-3">
                                        <input type="date" class="form-control" id="date" name="date" value="{{$data->booking_date}}" placeholder="Date of OT booking" required>
...
<script>
    ...
    var timetable_url ="{{route('OT.ottimetable_2')}}";
</script>
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (status === "1") {
                            ...
                            const booktimedArray = response.booktimed;
                            ...
                            if (booktimedArray && booktimedArray.length > 0) {
                                // Display time slots as toggle buttons
                                const timeDiv = $('#time');
                                
                                timeDiv.html(timeSlots.map((slot, index) => {
                                    // Check if the current slot is booked in any of the booking times
                                    const isBooked = booktimedArray.some(bookingTime => {
                                        const [start, end] = bookingTime.split(" - ");
                                        return slot >= start && slot <= end;
                                    });

                                    // Add 'active' class to booked slots and disable them
                                    const buttonClass = isBooked ? 'toggle-button active disabled btn-danger' : 'toggle-button';

                                    // Add 'data-slot' attribute with the slot value
                                    const displaySlot = isBooked ? slot : slot.split(' - ')[0]; // Display only the start time

                                    return `<button id="timeSlot${index}" class="${buttonClass} m-2 btn " data-slot="${slot}" ${isBooked ? 'disabled' : ''}>${displaySlot}</button>`;
                                }).join(""));
                            } else{
                                // Display time slots as toggle buttons
                                const timeDiv = $('#time');
                                timeDiv.html(timeSlots.map((slot, index) => {
                                    ...
                                    // Add 'data-slot' attribute with the slot value
                                    return `<button id="timeSlot${index}" class="${buttonClass} m-2 btn " data-slot="${slot}" >${slot}</button>`;
                                }).join(""));
                            }
                           ...
                           // Set start time
                            $('.toggle-button').on('click', function (event) {
                                if (!$(this).hasClass('disabled')) {
                                    ...
                                    if (startTime === null) {
                                        ...
                                        var isRangeBooked = booktimedArray.some(bookingTime => {
                                                const [start, end] = bookingTime.split(" - ");
                            ...
                            messageDiv.html("OT not Available. Please select another date.");
