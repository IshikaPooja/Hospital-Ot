...
// ottimetable
       ...
       public function ottimetable(Request $request){
        ...
        $timesloat = OTTime::where('ot_id',$id)->where('day',$dayOfWeek)->where('status','0')->first();

        $selectedbooking = OTBook::where('ot_number',$id)
        ->where('booking_date',$request->date)->get();
      
        $booktimed =   $selectedbooking->pluck('booking_time')->toArray();
   
        if($timesloat){
            return response(['status'=>'1', 'timesloat' => $timesloat, 'booktimed' => $booktimed ,'']);   
        }else{
       ...
       public function ottimetable_2(Request $request){
        $id = $request->id;
        
        $otbook_id = $request->otbook;
        $date = new \DateTime($request->date);         
        $dayOfWeek = $date->format('N'); 
        ...
        $selectedbooking = OTBook::where('ot_number',$id)
        ->where('booking_date',$request->date)->whereNotIn('id',  [$otbook_id])->get();
